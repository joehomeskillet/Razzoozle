...
## ADDENDUM (User-Direktive 2026-07-14, nach SDD-Abnahme): klassenEnabled-Gating

Die Fächer-ZUORDNUNG ist nur im Lehrermodus (Klassenmodus, `config.klassenEnabled`) aktiv:
- Labels-Definitions-Tab: `roleGate: "admin"` **UND** `gated: "klassenEnabled"` (beide Gates).
- `label:assign` server-seitig: bei `klassen_enabled = false` ablehnen (warn! + error-Emit) — Contract-Doku entsprechend.
- Flächen-WPs (L3-L5): Tagging-Controls, Label-Chips und Pill-Filter nur rendern, wenn `klassenEnabled === true`
  (Quiz-Editor-Baum: aus `useManagerStore(s=>s.config)`, NICHT useConfig — bekannter Routen-Gotcha).
- Betroffene Akzeptanzkriterien: jedes UI-WP testet zusätzlich den AUS-Zustand (keine Label-UI sichtbar).
