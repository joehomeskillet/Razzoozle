...
| **Required changes** | 1. Add/verify PageHeader. 2. Keep rank tabs/accordion (no duplication; use existing pattern). 3. Within each rank, replace oversized cards with compact ListRow or achievement-specific row: name, description, trigger/condition (abbreviated), threshold, bonus points, actions (edit, delete). 4. Replace edit panel with inline expansion, side panel, or modal. 5. Extend SettingRow (LabelRow/ToggleField) with restart-badge + status slots (title, description, control aligned right on wide, stacked on narrow) and apply to rank settings. 6. Add restart-required badge if applicable. 7. Implement dirty-state save bar (reset/save buttons). 8. Document reset scope: one achievement, one rank, or all defaults. 9. Preserve achievement logic server-side (no frontend evaluation). |
...
| **Status** | **TODO** — WP5 (settings consolidation) after SettingRow extended in WP2. |
...
| **Target primitives** | PageHeader, SectionCard (or two-column layout), collapsible/sticky preview, extended SettingRow pattern (LabelRow/ToggleField in components/ui/), consistent color-field UI, clear reset scope (unsaved / preset / defaults) |
...
| **WP5** (Settings) | Modus, KI, Design, Achievements, Satellit | Migrate all settings pages to extended SettingRow + ActionFooter (WP2 extensions must resolve first) | SettingRow extension (restart-badge + status slots) (WP2 dependency), Satellit scope decision | High (largest scope) |
...
1. **SettingRow API extension (WP2 → WP5 dependency):** Must be approved before Modus/KI/Design/Achievements can migrate. Recommend component unit tests + usage example.
