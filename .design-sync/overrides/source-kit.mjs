...
// forked from design-sync lib/source-kit.mjs — synth-entry must honor
// componentSrcMap:null excludes at the FILE level: excluded app screens
// (AnimatedErrorPage chain) transitively import src/i18n.ts whose top-level
// await cannot bundle into the IIFE.
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // PascalCase basename alongside the named exports.
    writeFileSync(
      entry,
      comps
        .map((p) => {
          const name = basename(p).replace(/\.(tsx|jsx)$/, '');
          const spec = JSON.stringify(p);
          const hasDefault = /export\s+default\s/.test(readText(p) ?? '');
          return (hasDefault ? `export { default as ${name} } from ${spec};\n` : '') + `export * from ${spec};`;
        })
        .join('\n') + '\n',
    );
    synthEntry = true;
