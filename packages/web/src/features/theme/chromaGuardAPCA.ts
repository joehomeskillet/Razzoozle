...
export class ChromaGuardAPCA {
   ...
   */
  public static calculateAPCAScore(txtY: number, bgY: number): number {
   ...
   */
  public static healContrast(
    fgRgb: [number, number, number],
    bgRgb: [number, number, number],
    targetLc = 60
  ): APCAContrastResult {
    const fgY = this.relativeLuminance(fgRgb[0], fgRgb[1], fgRgb[2])
    const bgY = this.relativeLuminance(bgRgb[0], bgRgb[1], bgRgb[2])
    const scoreLc = this.calculateAPCAScore(fgY, bgY)
