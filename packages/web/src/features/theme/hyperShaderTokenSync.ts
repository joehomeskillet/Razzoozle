 ...
 */
export class HyperShaderTokenSync {
  private static parseHexOrRgb(colorStr: string): { r: number; g: number; b: number; a: number } {
   ...
   */
  public static getTokenFloat32Buffer(token: CssTokenName): Float32Array {
    ...
    const { r, g, b, a } = this.parseHexOrRgb(rawVal || "#7c3aed")
   ...
   */
  public static getTokenCanvasRgba(token: CssTokenName, alpha = 1.0): string {
    const buf = this.getTokenFloat32Buffer(token)
