// Browser-safe copy of the socket-side text-match utility (see the type-answer
// spec §5.1). Pure string operations, no Node.js import, so it runs in the
// browser. The web package keeps its OWN copy rather than importing across the
// package boundary; the logic must mirror `socket/src/services/game/text-match.ts`
// exactly so the manager's results view classifies a submission identically to
// the server's authoritative scoring.
...
// for `matchMode === "normalized"` matching and for the text histogram bucket
// key, so counts stay accurate across case/accent variants.
...
// validator leaves matchMode optional).
