...
// One-time, idempotent seeding of the brand assets baked into the image
// (source/branding → /app/branding via BRANDING_PATH). Every copy is guarded by
// copyIfMissing, so re-running on an existing config volume is a no-op and a
// manager's edits to a template/background/og/logo are never clobbered. Does NOT
// touch the ACTIVE theme (config/theme/theme.json): presets are merely offered in
// the picker; the live theme stays whatever exists (default).
const seedBrandingAssets = (): void => {
  ...
  //    (the preset's `logo` points at /theme/rahoot-logo.svg).
...
export const initConfig = () => {
  ...
  // Seed the baked-in brand presets + assets last (the dirs above —
