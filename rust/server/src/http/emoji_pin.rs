...
pub async fn handle_emoji_pin_set() -> Json<Vec<EmojiPinSetEntry>> {
    let entries: Vec<EmojiPinSetEntry> = EMOJI_PIN_SET
        .iter()
        .map(|(emoji, label)| EmojiPinSetEntry { emoji, label })
        .collect();
    Json(entries)
}
...
mod tests {
    ...
    fn test_emoji_set_size() {
        assert!(
            EMOJI_PIN_SET.len() >= 256,
            "EMOJI_PIN_SET must have >= 256 entries, found {}",
            EMOJI_PIN_SET.len()
        );
    }
    ...
    fn test_multi_codepoint_pin_parsing() {
        ...
        assert!(
            EMOJI_PIN_SET.iter().any(|(e, _)| e == &spinne),
            "🕷️ not found in set"
        );
        assert!(
            EMOJI_PIN_SET.iter().any(|(e, _)| e == &flugzeug),
            "✈️ not found in set"
        );
        ...
        assert!(
            is_valid_pin(&pin),
            "PIN with multi-codepoint emojis should be valid"
        );
    ...
    fn test_symbols_of_extraction() {
        ...
        if let Some(symbols) = symbols_of(&pin) {
            ...
            for symbol in symbols {
                assert!(
                    EMOJI_PIN_SET.iter().any(|(e, _)| e == &symbol),
                    "Symbol should be in set: {}",
                    symbol
                );
            }
        } else {
