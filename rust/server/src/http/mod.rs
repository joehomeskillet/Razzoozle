...
/// `http::skeleton`).
pub async fn authorize_admin_request(headers: &HeaderMap, db_pool: &Option<sqlx::PgPool>) -> bool {
...
pub fn router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/api/quizz/:id/check-answer",
            post(solo::handle_check_answer),
        )
        .route("/api/quizz/:id/solo-score", post(solo::handle_solo_score))
        .route("/api/quizz/:id/study", get(solo::handle_get_quiz_study))
        .route(
            "/api/quizz/:id/practice-score",
            post(solo::handle_practice_score),
        )
        .route(
            "/api/templates",
            get(templates::handle_list_templates).post(templates::handle_create_template),
        )
        .route(
            "/api/templates/:id",
            get(templates::handle_get_template)
                .put(templates::handle_update_template)
                .delete(templates::handle_delete_template),
        )
        .route(
            "/api/templates/create-from",
            post(templates::handle_create_from_template),
        )
        .route(
            "/api/assignment",
            post(assignments::handle_create_assignment),
        )
        .route(
            "/api/assignment/:id",
            get(assignments::handle_get_assignment),
        )
        .route(
            "/api/assignment/:id/results",
            get(assignments::handle_get_assignment_results),
        )
        .route(
            "/api/assignment/:id/validate-pin",
            post(assignments::handle_validate_pin),
        )
        ...
        .route(
            "/api/skeleton/export",
            get(skeleton::handle_skeleton_export),
        )
        .route(
            "/api/skeleton/import",
        ...
        .route(
            ...
            post(client_events::handle_client_events),
        )
        .route(
            "/api/plugins/import",
            post(plugins::handle_plugin_import).layer(axum::extract::DefaultBodyLimit::disable()),
        )
        .route(
        ...
        .route(
            ...
            get(observability::handle_observability_events),
        )
        .route(
            "/api/v1/observability/schema",
            get(observability::handle_observability_schema),
        )
        .route(
            "/api/v1/observability/logs/server",
            get(logs::handle_logs_server),
        )
        .route(
            "/api/v1/observability/logs/client",
            get(logs::handle_logs_client),
        )
        ...
        .route(
            "/sw.js",
            get(|| async { static_files::handle_spa_static("sw.js").await }),
        )
        .route(
            "/registerSW.js",
            get(|| async { static_files::handle_spa_static("registerSW.js").await }),
        )
        .route(
            "/manifest.webmanifest",
            get(|| async { static_files::handle_spa_static("manifest.webmanifest").await }),
        )
